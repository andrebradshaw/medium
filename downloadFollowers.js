/*
 Run this on a user's follower page: https://medium.com/@somepersonofsomekind/followers
 Output is a TSV because TSV > CSV
*/
...
async function getFollowers(user, nextid, offset) {
  var res = await fetch("https://medium.com/_/api/users/" + user + "/profile/stream?limit=25&to=" + nextid + "&source=followers&page=" + offset, {
    "credentials": "include",
    "headers": {
      "accept": "application/json",
      "accept-language": "en-US,en;q=0.9",
      "content-type": "application/json",
      "x-client-date": "1563908795737",
      "x-obvious-cid": "web",
      "x-xsrf-token": "1"
    },
...
async function loopThoughAllFollowers() {
  ...
  for (var i = 2; i < 5000; i++) {
    ...
    if (res.payload.references.User) {
      ...
      console.log(loop)
      loop.forEach(el => {
        if (containArr.every(itm => itm.userId != el.userId)) containArr.push(el)
      });
